...
function searchAnimepahe(query) {
  return fetch(`${BASE}/api?m=search&q=${encodeURIComponent(query)}`)
    .then(r => (r.ok ? r.json() : null))
    .then((json) => {
...
function getEpisodeLink(animeUrl, episodeNum) {
  ...
  function fetchPage(page) {
    return fetch(`${BASE}/api?m=release&id=${animeId}&sort=episode_asc&page=${page}`)
      .then(r => (r.ok ? r.json() : null));
  }
...
function resolvePlayUrl(entry) {
  return fetch(entry.kwikUrl)
    ...
    .then((html) => {
      ...
      const audioType = entry.audio === 'eng' ? 'Dub' : 'Hardsub';
      return {
            ...
            .then((playUrl) => {
              ...
              return fetch(playUrl)
                ...
                .then((html) => {
                  ...
                  const entries = buttonMatches.map(extractKwik).filter(Boolean);
